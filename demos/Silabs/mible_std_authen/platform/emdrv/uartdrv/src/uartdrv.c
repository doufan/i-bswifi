                            ...
                            UARTDRV_Buffer_t *buffer)
{
  ...
  DBG(YELLOW" StartReceiveDma: handle->rxDmaActive: %d\r\n"NONE, handle->rxDmaActive);
                               ...
                               void *userParam)
{
  ...
  DBG(YELLOW" ReceiveDmaComplete: before status: %d\r\n"NONE, status);
  ...
  DBG(YELLOW" ReceiveDmaComplete: after status: %d\r\n"NONE, status);
  ...
  if (handle->rxQueue->used > 0) {
    ...
    DBG(YELLOW" ReceiveDmaComplete:  into else\r\n"NONE);
  }
  CORE_EXIT_ATOMIC();
  DBG(YELLOW" ReceiveDmaComplete:  handle->rxDmaActive: %d\r\n"NONE, handle->rxDmaActive);
  return true;
}
                             ...
                             UARTDRV_Buffer_FifoQueue_t * txQueue)
{
  ...
  DBG(YELLOW" InitializeQueues:  handle->rxDmaActive: %d\r\n"NONE, handle->rxDmaActive);
}
...
Ecode_t UARTDRV_Abort(UARTDRV_Handle_t handle, UARTDRV_AbortType_t type)
{
  ...
  CORE_EXIT_ATOMIC();
  DBG(YELLOW" UARTDRV_Abort:  handle->rxDmaActive: %d\r\n"NONE, handle->rxDmaActive);
  return ECODE_EMDRV_UARTDRV_OK;
}
